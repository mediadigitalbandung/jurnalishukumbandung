...
export default async function HomePage() {
  ...
  const [articles, categories, trendingArticles, trendingTags, breakingLatest, headlineData, relatedArticles, liveSessions] = await Promise.all([
    // Main articles — exclude auto-generated
    ...
    // Breaking News — exclude auto-generated
    prisma.article.findMany({
      where: {
        status: "PUBLISHED",
    ...
    // Headline — exclude auto-generated
    prisma.article.findMany({
      where: {
        status: "PUBLISHED",
        isAutoGenerated: false,
    ...
    // Berita Terkait — ONLY auto-generated articles
    prisma.article.findMany({
      where: { status: "PUBLISHED", isAutoGenerated: true },
      select: articleSelect,
      orderBy: { publishedAt: "desc" },
      take: 10,
    }),
    // Live sessions — yang sedang LIVE atau dijadwalkan dalam 24 jam ke depan
  ...
  return (
    <>
      ...
      {/* Berita Terkait — auto-generated articles only */}
        ...
        <section className="bg-surface-secondary py-8">
          <div className="container-main">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/berita" className="text-sm font-semibold text-goto-green hover:text-goto-dark transition-colors">
                Lihat Semua →
              </Link>
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {relatedArticles.slice(0, 8).map((a: any) => (
                <ArticleCard
                  key={a.id}
                  title={a.title}
                  slug={a.slug}
