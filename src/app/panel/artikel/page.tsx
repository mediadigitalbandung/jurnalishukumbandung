...
export default function ArtikelPage() {
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      ...
      // Creators only see their own articles, always exclude auto-generated
      let url = `/api/articles?page=${page}&limit=20&status=${filterStatus}&autoGenerated=false`;
      if (isCreator) {
        url += `&authorId=${userId}`;
      }
  ...
  return (
    <div>
      ...
      {selectedIds.size > 0 && (
        <div className="sticky top-0 z-30 flex items-center justify-center gap-3 rounded-[12px] bg-surface-dark px-6 py-3 shadow-lg border border-white/10 mb-3">
