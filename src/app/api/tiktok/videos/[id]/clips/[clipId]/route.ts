...
const updateClipSchema = z.object({
  ...
  textX: z.number().min(0).max(100).nullable().optional(),
  textY: z.number().min(0).max(100).nullable().optional(),
  textFontSize: z.number().int().min(16).max(200).nullable().optional(),
  ...
  try {
    ...
    const updated = await prisma.tiktokClip.update({
      ...
      data: {
        ...
        ...(data.textX !== undefined && { textX: data.textX }),
        ...(data.textY !== undefined && { textY: data.textY }),
        ...(data.textFontSize !== undefined && { textFontSize: data.textFontSize }),
